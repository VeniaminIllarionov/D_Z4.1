...
def test_init(category):
    ...
    category.append_product(Product('Iphone 15', '512GB', 1555555, 222, 'Gray space'))
    assert category.products_dispaly == ['Samsung Galaxy C23 Ultra, 180000.0 руб. Остаток: 5 шт.',
                                         'Iphone 15, 210000.0 руб. Остаток: 8 шт.',
                                         'Xiaomi Redmi Note 11, 31000.0 руб. Остаток: 14 шт.',
    ...
    with pytest.raises(ValueError, match="Товар с нулевым количеством не может быть добавлен"):
        category.append_product(Product('simens', 'good phone', 10_000, 0, "Blue"))
    assert category.average_price() == 43804.62
